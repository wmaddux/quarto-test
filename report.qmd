...
from rules import (
    ...
    memory_hwm_check, network_acceleration_check, version_consistency_check,
    config_drift_check  
)
...
# B. Cluster Assessment Narrative
if crit_count > 0:
    cluster_assessment = f"⚠️ This cluster requires **immediate intervention**. {crit_count} critical issues were detected that may impact data durability or service availability."
elif warn_count > 3:
    cluster_assessment = f"⚡ The cluster is stable but showing signs of **configuration drift or resource pressure**. {warn_count} warnings suggest optimization is required."
else:
    cluster_assessment = "✅ The cluster is currently **healthy and operating within optimal parameters**. No significant deviations from Aerospike best practices were identified."

# C. Executive Summary Narrative
is_sc = "SC" in feature_list
mode_suffix = "Strong Consistency (SC) deployment" if is_sc else "Available/Partition-tolerant (AP) deployment"

summary_narrative = (
    f"This diagnostic evaluates the **{cluster_display}** environment, currently running Aerospike **{server_version}** "
    f"across a **{node_count}-node** footprint. Optimized as a **{mode_suffix}**, the findings below prioritize "
    f"{'partition safety and transaction integrity' if is_sc else 'throughput efficiency and resource headroom'}."
)

if "XDR" in feature_list:
    summary_narrative += " Cross-Datacenter Replication (XDR) is active, adding requirements for inter-cluster bandwidth monitoring."

if crit_count > 0:
    summary_narrative += " **Action Required:** Immediate focus should be placed on remediating the critical failures identified in the primary findings."
```
